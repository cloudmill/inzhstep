...
import {
  createDebounce,
  createThrottle,
} from "./assets/scripts/optimization.js";
import "./assets/scripts/parsley";
import "./assets/scripts/inputmask";
      ...
      $("[data-type=form]").submit(function (e) {
        ...
        $.ajax({
          ...
          success: function (r) {
            if (r.success) {
              $.fancybox.open(
                $(`[data-fancy-modal="${thisObj.data("modal-id")}"]`)
              );
            } else {
    ...
    // info slider
    {
      ...
      if (slider.length !== 0) {
        ...
        thumbsSwiper.on("click", (event) => {
          ...
          thumbsSlide
            .eq(event.clickedIndex)
            .addClass("product__thumbs-slide--active");
        });
  ...
  $(() => {
    ...
    if (header.length !== 0) {
      ...
      function openModal(headerModal) {
        ...
        header
          .find("." + headerModalCurrent)
          .addClass(headerModalCurrent + "--active");
      }
      ...
      function closeModal() {
        if (headerModalCurrent !== null) {
          ...
          header
            .find("." + headerModalCurrent)
            .removeClass(headerModalCurrent + "--active");
      ...
      const searchModalCloseButton = searchModal.find(
  ...
  $(() => {
    ...
    if (headerCatalog.length !== 0) {
      ...
      headerCatalogPanelLink.on("click", function (event) {
        ...
        $(".header-catalog__tabs-item").removeClass(
          "header-catalog__tabs-item--active"
        );

        headerCatalog
          .find(".panel__item")
          .eq(headerCatalogActiveIndex)
          .addClass("panel__item--active");
        // с учетом того, что .header-catalog на странице в единичном экземпляре
        $(".header-catalog__tabs-item")
          .eq(headerCatalogActiveIndex)
          .addClass("header-catalog__tabs-item--active");
      });
    }
  });
}
...
// header-menu
{
  $(() => {
    ...
    if (headerMenu.length !== 0) {
      ...
      headerMenuCatalog
        .find(".header-menu__catalog-button")
        .on("click", function () {
          headerMenuCatalog
            .find(".header-menu__catalog-dropdown")
            .slideToggle(650);
      ...
      const headerMenuSectionButton = headerMenuSection.find(
        ".header-menu__section-button"
      );

      headerMenuSectionButton.on("click", function () {
        const headerMenuSectionClicked = $(this).closest(
          ".header-menu__section"
        );

        if (headerMenuSectionClicked.hasClass("header-menu__section--active")) {
          headerMenuSectionClicked
            .find(".header-menu__section-dropdown")
            .slideUp(650);
          headerMenuSectionClicked.removeClass("header-menu__section--active");
        } else {
          $(".header-menu__section--active")
            .find(".header-menu__section-dropdown")
            .slideUp(650);
          $(".header-menu__section--active").removeClass(
            "header-menu__section--active"
          );
...
// nav-card
{
  ...
  $(() => {
    const mediaQuery = window.matchMedia(
      `(max-width: ${MAIN_BREAKPOINT - 1}px)`
    );
    if (mediaQuery.matches) {
      ...
      // slider (https://github.com/codrops/SlideOutBoxMenu/)
      {
        ...
        class Slide {
          constructor(el, settings) {
            ...
            // The title container.
            this.DOM.titleWrap =
              this.DOM.wrap.querySelector(".slide__title-wrap");
            // The details boxes.
          ...
          setCurrent(isCurrent = true) {
            this.DOM.el.classList[isCurrent ? "add" : "remove"](
              "slide--current"
            );
          }
          // Hide the slide.
        ...
        class Navigation {
          constructor(el, settings) {
            ...
            this.DOM.prevCtrl = this.DOM.el.querySelector(
              ".boxnav__item--prev"
            );
            this.DOM.nextCtrl = this.DOM.el.querySelector(
              ".boxnav__item--next"
            );
      ...
      // parallax
      {
        ...
        $(window).on("scroll", function () {
          if ($(this).scrollTop() <= top.offset().top + top.height()) {
            const parallax =
              ($(this).scrollTop() - background.offset().top) / PARALLAX_RATIO;
...
// panel
{
  $(() => {
    ...
    if (
      panelButtons.length &&
      matchMedia(`(min-width: ${MAIN_BREAKPOINT}px)`).matches
    ) {
      ...
      panelButtons.on("click", function (event) {
        ...
        try {
          $("html, body").animate(
            {
              scrollTop:
                $(`.sections__block:nth-child(${activeIndex + 1})`).offset()
                  .top - Math.max(100, getPanelDetectTop() - 1),
            },
            MOVE_DURATION
          );
        } catch {}
      ...
      window.addEventListener(
        "resize",
        resizeDebounce(updateScrollY, 1000 / RESIZE_FPS)
      );
      window.addEventListener(
        "scroll",
        scrollThrottle(updateScrollY, 1000 / SCROLL_FPS)
      );
      ...
      window.addEventListener(
        "resize",
        resizeDebounce(updateBlocksY, 1000 / RESIZE_FPS)
      );
      window.addEventListener(
        "scroll",
        scrollThrottle(updateBlocksY, 1000 / SCROLL_FPS)
      );

      // panel active item & sections active block
      function getAverageBlockHeight() {
        const blocks = document.querySelectorAll(".sections__block");
        const blocksHeight = Array.from(blocks).map(
          (block) => block.getBoundingClientRect().height
        );
        const allBlocksHeight = blocksHeight.reduce(
          (allHeight, blockHeight) => allHeight + blockHeight
        );
      ...
      function getPanelActiveItemIndex() {
        let activeIndex = blocksY.findIndex(
          (blockY) => getPanelDetectY() < blockY
        );
        if (activeIndex < 0) {
      ...
      function setPanelActiveItem(index) {
        ...
        $(`[data-sections] .panel__item:nth-child(${index})`).addClass(
          "panel__item--active"
        );
      }
      function setSectionsActiveBlock(index) {
        $(".sections__block").removeClass("sections__block--active");
        $(`.sections__block:nth-child(${index})`).addClass(
          "sections__block--active"
        );
      }
      ...
      window.addEventListener(
        "resize",
        resizeDebounce(updatePanelActiveItem, 1000 / RESIZE_FPS)
      );
      window.addEventListener(
        "scroll",
        scrollThrottle(updatePanelActiveItem, 1000 / SCROLL_FPS)
      );
    }
  });
}
